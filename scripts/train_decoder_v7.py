"""
Decoder v7 — GRU-based RNN neural decoder.

Key changes from v6 (MLP):
  1. GRU processes bins sequentially, preserving temporal ordering
  2. ~467K params (smaller than v6 MLP), less overfitting risk
  3. Same preprocessing: bandpass + spike counts at 3/4/5σ (Track A, 192 features/bin)
  4. ONNX export with transposed input (1, 192, 30) matching device convention
...
def process_labels_tensor(y_raw, device):
    """Tensor version of process_labels for training loop."""
    y_joy = y_raw[:, :4] / 32767.0
    y_trig = y_raw[:, 10:12] / 32767.0
    y_trig = torch.clamp(y_trig, 0, 1)
    y_btn = (y_raw[:, 4:10] > 16000).float()
    y_gate = (y_btn.sum(1) == 0).float().unsqueeze(1)
    return y_joy.to(device), y_trig.to(device), y_gate.to(device)


# ── Sequence Dataset ─────────────────────────────────────────────────────

class SequenceDataset(Dataset):
    """Returns (sequence, label) pairs. No cross-file sequences."""
    def __init__(self, feats_list, labels_list, seq_len=SEQ_LEN, stride=1):
        ...
        for fi, feats in enumerate(feats_list):
            ...
            if n_bins < seq_len:
                continue
            for start in range(0, n_bins - seq_len + 1, stride):
...
def train_rnn(feats_tr, labels_tr, feats_val, labels_val, epochs=300):
    """Train GRU decoder with proper evaluation."""
    ...
    for ep in range(epochs):
        ...
        for xb, yb in train_dl:
            ...
            y_joy, y_trig, y_gate = process_labels_tensor(yb, DEVICE)
            joy_p, trig_p, gate_p = model(xb)
            loss = mse_fn(joy_p, y_joy) + mse_fn(trig_p, y_trig) + 0.5 * bce_fn(gate_p, y_gate)
...
def main():
    t_start = time.time()
    log(f"Decoder v7 (GRU) — Device: {DEVICE}")
